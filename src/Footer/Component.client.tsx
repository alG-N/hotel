...
export function FooterClient({ data }: FooterClientProps) {
  ...
  // Extract data with translated defaults
  const subscribeTitle = data?.subscribeTitle || t('Nhận tin tức & ưu đãi mới nhất', 'Get the latest news & discounts')
  const subscribeSubtitle = data?.subscribeSubtitle || t('Đăng ký nhận bản tin để cập nhật thông tin', 'Subscribe to our newsletter to stay updated')
  const subscribeButtonText = data?.subscribeButtonText || t('Đăng ký', 'Subscribe')
  const subscribeButtonLink = data?.subscribeButtonLink || '/subscribe'
  const addresses = data?.addresses || []
  const phone = data?.phone || '+84 777 4340'
  ...
  const borderColor = data?.borderColor || '#e5e5e5'
  const subscribeBackgroundColor = data?.subscribeBackgroundColor || '#f5f5f5'
  const titleFont = data?.titleFont || 'Georgia, serif'
  const bodyFont = data?.bodyFont || 'system-ui, -apple-system, sans-serif'
  const titleFontWeight = data?.titleFontWeight || '400'
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            {data?.logo && typeof data.logo === 'object' && (
              <div className="flex-shrink-0">
                <Link href="/">
                  <Media resource={data.logo} imgClassName="w-16 h-16 object-contain" />
                </Link>
              </div>
            )}
